...
function MovieDetail({ db, auth }) {
            ...
            if (ownerSnap.exists()) {
              ...
              if (auth.currentUser) {
                ...
                if (currentUserSnap.exists()) {
                  ...
                  setIsFollowing(following.includes(movieData.ownerId));
                }
              }
            }
          }
  ...
  const handleAddEntry = async (e) => {
    ...
    try {
      ...
      if (movie.ownerId !== auth.currentUser.uid) {
  ...
  const handleFollow = async () => {
    if (!auth.currentUser || !movie || !movie.ownerId) return;
    try {
      const currentUserRef = doc(db, "users", auth.currentUser.uid);
      const ownerRef = doc(db, "users", movie.ownerId);

      await updateDoc(currentUserRef, {
        following: arrayUnion(movie.ownerId)
      });
  ...
  const handleUnfollow = async () => {
    ...
    try {
      ...
      await updateDoc(currentUserRef, {
        following: arrayRemove(movie.ownerId)
      });
      await updateDoc(ownerRef, {
  ...
  if (!movie) {
    return <div className="text-center mt-5">Película no encontrada.</div>;
  }

  return (
    <div className="container mt-5">
      <div className="card p-4 shadow-sm mb-4">
        <h2 className="card-title text-center mb-4">{movie.title}</h2>
        ...
        <p className="card-text"><strong>Visitas:</strong> {movie.views || 0}</p>
        {movie.ownerId && (
          <p className="card-text">
            <strong>Propietario:</strong>
            <Link to={`/profile/${movie.ownerId}`} className="btn btn-sm btn-info ms-2">
