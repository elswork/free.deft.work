...
function VideoDetail({ db, auth }) {
    ...
    const fetchVideo = async () => {
      try {
        ...
        if (docSnap.exists()) {
          ...
          setVideo(videoData);
          setIsTopContent(videoData.isTopContent || false);
          setTopOrder(videoData.topOrder || '');
  ...
  const handleUpdateTopContent = async () => {
    if (!auth.currentUser || !video || auth.currentUser.uid !== video.ownerId) return;
  ...
  return (
    <div className="container mt-5">
      <div className="card p-4 shadow-sm mb-4">
        ...
        {auth.currentUser && video.ownerId === auth.currentUser.uid && (
