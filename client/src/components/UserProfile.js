...
function UserProfile({ auth, db, storage }) {
  const { alias } = useParams(); // Changed from userId to alias
  ...
  const isCurrentUserProfile = auth.currentUser && profile && auth.currentUser.uid === profile.uid;
    ...
    const fetchProfile = async () => {
      ...
      try {
        ...
        if (!querySnapshot.empty) {
          ...
          // 2. If not found by alias, try to fetch by UID
